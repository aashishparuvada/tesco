"""Load every CSV in data/ into the raw_data schema.

The table definitions are not hand-written: each CSV is profiled first, a
column type is inferred from the values it actually holds, and the resulting
DDL is both executed against the database and written to
scripts/sql/raw_data/ddl.sql for reference.

Adding a new CSV to data/ is therefore the only step needed to get a new
raw_data table -- no change to this script is required.
...
def build_ddl(tables: dict[str, list[ColumnStats]]) -> str:
    header = (
        "-- AUTO-GENERATED by scripts/python/load_data.py -- do not edit by hand.\n"
...
def main() -> int:
    ...
    try:
        with conn, conn.cursor() as cursor:
            print(f"\nCreating schema {SCHEMA} if needed...")
            cursor.execute(
    ...
    print(f"\nAll {len(csv_files)} table(s) loaded into {SCHEMA} ({total:,} rows).")
    return 0
